\begin{section}{Implementation}
	\begin{subsection}{Used Metric}
		...
		\end{itemize}
		Since some labels do not appear very often, it can happen that for a particular label no positive (or negative) sample is contained in the test data set. This is problematic when trying to calculate the balanced accuracy for that label. To avoid division by zero, the sensitivity (or specificity respectively) for this label is then set to 0.5 (chance level).
